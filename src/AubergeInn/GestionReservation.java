...
public class GestionReservation {
    ...
    public void afficherClient(int idClient) throws IFT287Exception {
        try {
            ...
            List<TupleReservation> lRes = tableReservations.getReservationClient(idClient);
            ...
            for (TupleReservation r : lRes) {

                toPrint.append("------\n");
                toPrint.append("Identifiant de la chambre: " + r.getM_idChambre() + "\n");
                toPrint.append("Date de début: " + r.getM_datedebut().toString() + "\n");
                toPrint.append("Date de Fin: " + r.getM_dateFin().toString() + "\n");

                TupleChambre tupleChambre = tableChambres.getChambre(r.getM_idChambre());

                toPrint.append("Prix: " + tupleChambre.getPrixTotal() + "$\n");
            }
